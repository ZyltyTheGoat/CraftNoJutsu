...
public class NinjutsuWheelScreen extends AbstractContainerScreen<NinjutsuWheelMenu> implements NarutoModScreens.ScreenAccessor {
	...
	public NinjutsuWheelScreen(NinjutsuWheelMenu container, Inventory inventory, Component text) {
	...
	private void initializePages() {
		...
		Map<String, JutsuData> jutsus = JutsuRegistry.getJutsusByType("NINJUTSU");
		List<String> jutsuIds = new ArrayList<>();
		List<String> jutsuNames = new ArrayList<>();
		for (Map.Entry<String, JutsuData> entry : jutsus.entrySet()) {
			JutsuData jutsu = entry.getValue();
			String jutsuId = entry.getKey();
			if (jutsu.getType().equalsIgnoreCase("NINJUTSU") && unlockedJutsus.contains(jutsuId)) {
				jutsuIds.add(jutsuId);
				jutsuNames.add(jutsu.getName());
			}
		}
		if (!jutsuIds.isEmpty()) {
			pages.add(new Page("Ninjutsu", "Ninjutsu", jutsuIds, jutsuNames));
		}
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		renderIcon(guiGraphics, centerX, centerY);
	...
	private void renderIcon(GuiGraphics guiGraphics, int centerX, int centerY) {
		String iconName = "icon_ninjutsu";
